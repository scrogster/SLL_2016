...
$$
\begin{aligned}
\text{logit}\left(\phi_{i,t}\right)=\lambda_{1}+\lambda_{2}\text{grass}_{i}+\lambda_{3}\text{fire}_{i}+\lambda_{4}\text{fire}_{i}^{2}+\lambda_{5}\text{graze}_{i} + \lambda_{6}\text{graze}_{i}^{2}+\epsilon_{\text{persist}} 
\end{aligned}
$$
...
\newpage
```{r occparamtable, echo=FALSE, message=FALSE, results = 'asis', eval=FALSE}
require(knitr)
require(dplyr)
require(tidyr)
load("fitted_model.Rdata")

df1<-data.frame(out$sims.list$B) %>%
	rename(Init_Intercept=X1, Init_Grassland=X2, Init_Clay=X3, Init_Roadside=X4) %>%
	gather() %>%
	mutate(key=factor(key, levels=c("Init_Intercept", "Init_Grassland", "Init_Clay","Init_Roadside")))

df2<-data.frame(out$sims.list$C) %>%
	rename(Persist_Intercept=X1, 
				 Persist_Grassland=X2, 
				 Persist_Fire=X3, 
				 Persist_Fire2=X4, 
				 Persist_Grazing=X5, 
				 Persist_Grazing2=X6) %>%
...
tmp<-df2 %>% filter(key=="Persist_Fire") 
tmp2<-df2 %>% filter(key=="Persist_Fire2") 

plot(tmp$value~tmp2$value)

df3<-data.frame(out$sims.list$D) %>%
	rename(Col_Intercept=X1, Col_Grassland=X2) %>%
	gather() %>%
	mutate(key=factor(key, levels=c("Col_Intercept", "Col_Grassland")))  

df_all<-rbind(df1, df2, df3)

posterior<-df_all %>%
	group_by(key) %>%
	summarise(post.mean=mean(value), post.sd=sd(value), post.median=quantile(value, 0.5),
	post.lwr=quantile(value, 0.025),
	post.upp=quantile(value, 0.975)) %>%
	separate(key, c("Process", "Parameter"), sep="_") %>%
	mutate(Process = replace(Process, Process=="Init", "Initial occupancy")) %>%
	mutate(Process = replace(Process, Process=="Persist", "Persistence"))%>%
	mutate(Process = replace(Process, Process=="Col", "Colonisation"))

#row.names(posterior)<-paste0(rep("$\\alpha_{", 18), 1:18,"}$")
names(posterior)<-c("Process", "Parameter", "mean", "sd", "median", "CI lower", "CI upper")


posterior<-data.frame(posterior)

kable(posterior, digits=3, row.names=FALSE,
			caption="Summary statistics for the posteriors of the parameters influencing probabilities of initial occupancy, persistence and colonisation by *D. impar* at grassland remnants in Western Victoria.")
```
...
detection_type=c(rep("lizard", 13), rep("indirect", 5))
...
kable(posterior, digits=3, align=c("l","l","c","c", "c", "c"), row.names=TRUE,
...
```{r, fig.cap="Posterior densities of regression parameters predicting log-odds of (A) initial occupancy, (B) annual probability of persistence, and (C) annual probability of colonisation by *Delma impar* in remnant grassland sites in Western Victoria.", fig.pos="H", echo=FALSE, cache=FALSE}
