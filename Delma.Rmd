...
For sloughed skins and eggs, the probability of detection $(p^{indirect}_{j})$ was related to Julian date, and the logarithm of the number of weeks since site establishmen as follows. 

$$
\begin{aligned}
\log \left(\dfrac{p^{indirect}_{j}}{1-p^{indirect}_{j}} \right) &=\alpha_{13}+ \\
...
meaning=c("$\\text{Intercept}$", "$\\cos2\\pi \\text{date}$", "$\\sin2\\pi \\text{date}$", 
					...
					"$\\text{hour}$", "$\\text{hour}^2$", "$\\text{log(siteage)}$",
